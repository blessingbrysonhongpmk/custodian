...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const navItems: { id: AdminNavTab; labelEn: string; labelTa: string; icon: React.ComponentType<{ className?: string }>; count?: number }[] = [
    { id: 'map', labelEn: 'Statewide Map', labelTa: 'வரைபடம்', icon: MapPin },
    { id: 'trees', labelEn: 'Trees Inventory', labelTa: 'மரங்கள்', icon: TreePine, count: trees.length },
    { id: 'dashboard', labelEn: 'Executive Dashboard', labelTa: 'கட்டுப்பாட்டகம்', icon: LayoutDashboard },
      ...
      <div className="flex flex-col lg:flex-row items-start gap-6">
        ...
        <aside className="w-full lg:w-64 shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm p-4 lg:sticky lg:top-20 space-y-4">
          <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
            <div className="w-10 h-10 rounded-xl bg-purple-900 text-white flex items-center justify-center text-lg font-black shrink-0 shadow-xs">
              <Building2 className="w-5 h-5" />
            </div>
            <div className="min-w-0">
              <span className="text-[10px] font-bold uppercase tracking-wider text-purple-900 bg-purple-100 px-2 py-0.5 rounded-full border border-purple-200">
                Administration
              </span>
              ...
              <span className="text-[10px] text-slate-400 font-mono block truncate">
                ADMIN-{currentUser.id.toUpperCase()}
              </span>
            </div>
          </div>
          ...
          <div className="px-2 py-0.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
              {language === 'ta' ? 'நிர்வாக தலைப்புகள்' : 'Admin Navigation'}
            </span>
          </div>
